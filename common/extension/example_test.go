...
func ExampleInteger() {
	// Valid input (string with decimal → truncated)
	v := extension.Integer("3.99")
	fmt.Println(v)

	// Valid input (float → truncated)
	v2 := extension.Integer(7.8)
	fmt.Println(v2)

	// Invalid input → falls back to default 0
	v3 := extension.Integer("oops")
	fmt.Println(v3)
...
func ExampleIntegerOr() {
	// Valid input
	v := extension.IntegerOr("42", -1)
	fmt.Println(v)

	// Invalid input → caller-supplied default -1 is returned
	v2 := extension.IntegerOr("oops", -1)
	fmt.Println(v2)
