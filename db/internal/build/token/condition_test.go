...
// File: db/internal/build/token/condition_test.go
...
func TestNewCondition(t *testing.T) {
	...
	t.Run("Operators", func(t *testing.T) {
		...
		t.Run("IN", func(t *testing.T) {
			c := token.NewCondition("type", token.In, []any{"A", "B"})
			sql, args := c.Render(d)
		...
		t.Run("BETWEEN", func(t *testing.T) {
			c := token.NewCondition("created_at", token.Between, []any{"2024-01-01", "2024-12-31"})
			sql, args := c.Render(d)
		...
		t.Run("IS NULL", func(t *testing.T) {
			c := token.NewConditionWith(token.ConditionTypeSimple, "deleted_at", token.IsNull)
			sql, args := c.Render(d)
		...
		t.Run("IS NOT NULL", func(t *testing.T) {
			c := token.NewConditionWith(token.ConditionTypeSimple, "deleted_at", token.IsNotNull)
			sql, args := c.Render(d)
		...
		t.Run("FromString", func(t *testing.T) {
			op, err := token.ParseConditionOperator("=")
			if err != nil || op != token.Equal {
				t.Errorf("expected Equal, got %q, err=%v", op, err)
			}
		})

		t.Run("FromOperator", func(t *testing.T) {
			op, err := token.ParseConditionOperator(token.In)
			if err != nil || op != token.In {
				t.Errorf("expected In, got %q, err=%v", op, err)
			}
		})

		t.Run("FromBytes", func(t *testing.T) {
			op, err := token.ParseConditionOperator([]byte(">="))
			if err != nil || op != token.GreaterThanOrEqual {
				t.Errorf("expected GreaterThanOrEqual, got %q, err=%v", op, err)
			}
		})

		t.Run("InvalidType", func(t *testing.T) {
			_, err := token.ParseConditionOperator(123)
	...
	t.Run("Render", func(t *testing.T) {
		...
		t.Run("EmptyColumn", func(t *testing.T) {
			c := &token.Condition{Operator: token.Equal, Value: "x"}
			sql, args := c.Render(d)
